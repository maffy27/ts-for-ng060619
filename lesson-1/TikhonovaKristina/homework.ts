...
// #1
const isInArray = (arr: ns[], ...args: ns[]): boolean => {
...
const summator = (...args: ns[]): number => {
    return args.reduce((sum: number, current: ns) => sum + +current, 0);
};
...
const getUnique = (...args: ns[]): ns[] => {
    ...
    args.forEach((argsItem: ns) => {
        if (resultArr.indexOf(argsItem) === -1) { resultArr.push(argsItem); }
    });
    return resultArr;
};
...
// #4
const toMatrix = (data: ns[], rowSize: number): Array<ns[]> => {
    const resultMatrix: Array<ns[]> = [];
